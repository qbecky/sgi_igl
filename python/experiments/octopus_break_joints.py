...
python octopus_break_joints.py --trial_number=0
"""
...
path_to_output_octopus = os.path.join(path_to_output, "octopus_break_joints/")

if not os.path.exists(path_to_output_octopus):
    os.makedirs(path_to_output_octopus)
...
from scipy.optimize import minimize, Bounds
from octopus_break_joints_settings import return_octopus_break_joints_experiment_settings
from snake_shapes import octopus_angles_generation
...
def obj_and_grad_params(
    ...
    edges, tail_lengths, tail_angles, broken_joint_ids, broken_joint_angles, n_angles_per_curve,
    ...
    Args:
        ...
        tail_lengths (list of Float): length of each tail.
        tail_angles (list of Float): angles of each tail to the first tail.
        ...
        n_angles_per_curve (list of int): Number of angles per curve in the snake.
    ...
    pos_, disc_angles = octopus_angles_generation(
        params.reshape(n_steps-close_snake_gait, -1), tail_lengths, tail_angles, broken_joint_ids, broken_joint_angles, n_angles_per_curve,
...
class OptimizationBookkeeper:
    ...
    def callback(self, x):
        ...
        pos_, disc_angles = octopus_angles_generation(
            params.reshape(self.n_steps-self.close_snake_gait, -1), self.tail_lengths, self.tail_angles, self.broken_joint_ids, self.broken_joint_angles, self.n_angles_per_curve,
        ...
        obj, grad_obj_g, grad_obj_pos_ = self.fun_obj_grad_g(g, pos_np_, self.gt, self.masses, self.a_weights, self.b_weights, self.edges, self.fun_anisotropy_dir, self.w_fit_scaled, self.w_energy_scaled)
...
def main(_):
    ...
    settings_dict = return_octopus_break_joints_experiment_settings(trial_number)
    n_angles_per_curve, n_ts = settings_dict["n_angles_per_curve"], settings_dict["n_ts"]
    edges = np.array(settings_dict["edges"])
    n_edges = edges.shape[0]
    rho, eps, tail_lengths, tail_angles, close_snake_gait = settings_dict['rho'], settings_dict['eps'], settings_dict['tail_lengths'], torch.tensor(settings_dict['tail_angles']), settings_dict['close_gait']
    ...
    n_tails = len(n_angles_per_curve)
    n_points_snake = n_angles + 2 * n_tails
    n_op_angles = n_angles - len(broken_joint_ids)
    
    average_tail_length = np.mean(tail_lengths)
    total_mass = rho * average_tail_length
    w_fit_scaled = w_fit / (average_tail_length ** 2)
    w_energy_scaled = w_energy / (total_mass * average_tail_length ** 2)
    ...
    pos_ = octopus_angles_generation(
    ...
    edge_lengths = np.linalg.norm(pos_[:, edges_torch[:, 1].numpy()] - pos_[:, edges_torch[:, 0].numpy()], axis=-1)
    ...
    save_path = os.path.join(path_to_output_octopus, "octopus_break_joints_init{}.json".format(tag_experiment))
    ...
    pos_opt_ = octopus_angles_generation(
        params_opt.reshape(n_ts-close_snake_gait, -1), tail_lengths, tail_angles, broken_joint_ids, broken_joint_angles, n_angles_per_curve,
        example_pos_, n_ts, close_gait=close_snake_gait
    ).detach().numpy()
    ...
    save_path_opt = os.path.join(path_to_output_octopus, "octopus_break_joints_opt{}.json".format(tag_experiment))
